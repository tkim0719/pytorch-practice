...
class pretrainedCNN(nn.Module):
  def __init__(self, n_hidden):
    ...
    self.cnn_layers = torch.load('pretrained_encoder.pt')['model']
    ...
    #transferring weights from pretrained encoder
  ...
  def top1_accuracy(self, logits, labels):
    ...
    return int(n_corrects) / float(logits.size(0)) * 100.
...
#returns minimum loss for both training and testing
def train_pretrain_valid(model, loaders, optimizer, writer, n_epochs, ckpt_path, device='cpu'):
  arr1 = []
  arr2 = []
...
def train_pretrain(model, loaders, optimizer, writer, n_epochs, ckpt_path, device='cpu'):
  arr1 = []
  arr2 = []
  arr3 = []
  arr4 = []
  ...
  #to calculate the minimum losses and maximum top1 and top3 accuracy
  arr1.sort()
...
def pretrain_main_val():
  ...
  for a in valid_hidden:
    for b in valid_optlr:
      ...
      parser.add_argument('--ckpt', type=str, default='./ckpt/pretrain_val%s' %a+str(b[1]) )
      ...
      if gpu == -1:
        ...
        DEVICE = gpu
      model =  pretrainedCNN(n_hidden).to(DEVICE)
      ...
      writer = SummaryWriter('./logs/pretrain/%s' % opt_str+str(n_hidden)+str(lr))
      temp1, temp2 = train_pretrain_valid(model, dataloaders, optimizer, writer, n_epochs, ckpt_path, DEVICE)
      min_loss.append(temp1)
      min_loss2.append(temp2)
  return min_loss, min_loss2

#main function for testing phase
def pretrain_main(in_hidden,in_optimizer, in_lr, number):
      parser = ArgumentParser()
      parser.add_argument('--hidden', type=int, default=in_hidden)
      parser.add_argument('--gpu', type=int, default=-1)
      parser.add_argument('--lr', type=float, default=in_lr)
      parser.add_argument('--batch', type=int, default=32)
      parser.add_argument('--ckpt', type=str, default='./ckpt/pretrain/{}'.format(number))
      ...
      optimizer = opt_class(model.parameters(), lr=lr)
      writer = SummaryWriter('./logs/pretrain/{}'.format(number))
      a,b,c,d,e,f = train_pretrain(model, dataloaders, optimizer, writer, n_epochs, ckpt_path, DEVICE)
      return a,b,c,d,e,f
...
if __name__=='__main__':
  #validation phase
  x,y = pretrain_main_val()
  n_hidden = 0
  optimizer = ''
  lr = 0.0
  print("validation for pretrained CNN")
  ...
  testarr6 = []
  while(x<5):
    a,b,c,d,e,f = pretrain_main(n_hidden,optimizer,lr,x)
    testarr.append(a)
    testarr2.append(b)
    testarr3.append(c)
    testarr4.append(d)
  ...
  print("train loss mean is {} and standard deviation is {}".format(cal_mean(testarr),cal_std(testarr)))
  print("evaluation loss mean is {} and standard deviation is {}".format(cal_mean(testarr4),cal_std(testarr4)))
  print("train top 1 accuracy Mean is {} and standard deviation is {}".format(cal_mean(testarr2),cal_std(testarr2)))
  print("evaluation top 1 accuracy Mean is {} and standard deviation is {}".format(cal_mean(testarr5),cal_std(testarr5)))
  print("train top 3 accuracy Mean is {} and standard deviation is {}".format(cal_mean(testarr3),cal_std(testarr3)))
  print("evaluation top 3 accuracy Mean is {} and standard deviation is {}".format(cal_mean(testarr6),cal_std(testarr6)))
